import { RankingProps } from '@features/contest-viewer/domain/model/scoreboardUi.model';
...
const Ranking = ({ contest }: RankingProps) => {
    ...
    <Stack spacing={7.5}>
      ...
      {contest.teams && (
        <Stack direction="row" spacing={10}>
          ...
          >
            {classment
              ?.filter((team) => classment.indexOf(team) < 8)
          ...
          <Stack spacing={3} justifyContent="center" sx={{ transform: 'skewX(-10deg)' }}>
            {classment
              ?.filter((team) => classment.indexOf(team) >= 8 && classment.indexOf(team) < 16)
              .map((team: Team, index: number) => (
                <ItemRanking key={team.id + '-match'} team={team} index={index} />
              ))}
          </Stack>
          <Stack spacing={3} justifyContent="center" sx={{ transform: 'skewX(-10deg)' }}>
            {classment
              ?.filter((team) => classment.indexOf(team) >= 16 && classment.indexOf(team) < 24)
              .map((team: Team, index: number) => (
                <ItemRanking key={team.id + '-match'} team={team} index={index} />
              ))}
          </Stack>
          <Stack spacing={3} justifyContent="center" sx={{ transform: 'skewX(-10deg)' }}>
            {classment
              ?.filter((team) => classment.indexOf(team) >= 24 && classment.indexOf(team) < 33)
              .map((team: Team, index: number) => (
                <ItemRanking key={team.id + '-match'} team={team} index={index} />
              ))}
          </Stack>
